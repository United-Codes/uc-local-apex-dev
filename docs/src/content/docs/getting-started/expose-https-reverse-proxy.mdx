...
<Aside type="caution" title="Not a hardening guide">
  ...
  ([`./install.sh --secure`](/products/uc-local-apex-dev/docs/getting-started/))
...
(Database REST API, MongoDB API, SQL Developer Web / Database Actions,
debug-to-screen). It also uses one password per workspace instead of the shared
`Welcome_1`.
