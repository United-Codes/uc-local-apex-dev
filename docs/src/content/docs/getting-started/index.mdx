...
When run with `--secure` (see below), it additionally disables the ORDS Database REST API, MongoDB API, SQL Developer Web / Database Actions and debug-to-screen before the restart, and records `FORCE_SECURE="true"` in `.env`.
...
This stack is built for **local** development and is intentionally insecure by
default. If you want to host it somewhere reachable as a **test** database (never
production), install with the `--secure` flag:

```bash
./install.sh --secure
```

Secure mode does exactly two things:

- **Disables the ORDS surfaces reachable without app authentication**: the ORDS
  Database REST API (`database.api.enabled`), the MongoDB API (`mongo.enabled`),
  SQL Developer Web / Database Actions (`feature.sdw`, `/ords/sql-developer`) and
  debug-to-screen (`debug.printDebugToScreen`). The APEX runtime and login keep
  working.
- **Uses the random `ORACLE_PASSWORD` for new workspaces** created with
  `create-user`, instead of the shared `Welcome_1`. (Equivalent to setting
  [`WORKSPACE_USE_INTERNAL_PASSWORD=true`](/products/uc-local-apex-dev/docs/getting-started/creating-users/).)

It sets `FORCE_SECURE="true"` in `.env`, so later `create-user` runs stay
consistent. Re-running `./install.sh --secure` on an existing install is safe.

**Secure mode does _not_** restrict network/port exposure and does _not_ rotate
the plaintext secrets in `.env` / `ords-secrets/conn_string.txt`. Before hosting:
put the stack behind a firewall + TLS reverse proxy, rotate those secrets, and
remove any stale `.env_old`.
