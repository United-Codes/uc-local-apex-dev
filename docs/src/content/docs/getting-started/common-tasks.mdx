...
The commands on this page that call `docker compose` also work with Podman. Replace `docker compose` with `podman compose`. The scripts of this project (`local-26ai.sh`, `./scripts/*.sh`) detect the engine, so you change nothing for those.
</Aside>
...
<Aside type="note">
The command runs online and never drops an object. You can run it again safely, because it skips the objects that are already compressed. It does not compress a unique index or a primary-key index with one column. Advanced index compression cannot remove duplicate values from these indexes.
</Aside>
