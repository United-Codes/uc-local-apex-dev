...
- DB 23.26.1.0 → 23.26.2.0. The datafiles are compatible. No dump and restore is necessary. The data dictionary does not upgrade automatically. See [Upgrade the data dictionary](#upgrade-the-data-dictionary).
- ORDS 26.1.0 → 26.1.2
- New scripts
...
The new binaries open the existing datafiles. Monitor the logs until the database shows `DATABASE IS READY TO USE!`. Also make sure that ORDS reports that it is initialized:
...
The image change does not upgrade the data dictionary. The database keeps the dictionary of the old release update.
...
The output must contain `missing_repairable=0` and `jvm_ok=yes`.
...
The `sys.utl_recomp.recomp_serial()` block in [Verify](#verify) does not correct this error. That procedure recompiles in a background job, and the macro call fails again.
