...
- ORDS 25.4 → 26.1
- New `./install.sh` one-shot installer that runs `setup.sh`, pulls images, brings the stack up, waits for the DB and ORDS to be ready, and runs `after-first-db-start.sh` non-interactively — only relevant for fresh installs, not for in-place migrations.
- Enhanced scripts
